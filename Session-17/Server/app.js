...
app.put("/updateuser", (req, res) => {
  ...
  fs.readFile("data.json", "utf-8", (err, data) => {
    if (err) {
      res.json({
        status: "failure",
        info: "Failed to access the data from file",
      });
    } else {
      data = JSON.parse(data);
        ...
        if (ele.id == req.body.id) {
          ...
          fs.writeFile("data.json", JSON.stringify(data), (err) => {
            if (err) {
              res.json({
                status: "failure",
                info: "Failed to Update new data in the file",
              });
            } else {
              res.json({
                status: "success",
                info: "Updated Data successfully",
              });
            }
          });
        }
...
app.delete("/delete/:id", (req, res) => {
  console.log(req.params);
  //implement logic to delete User from the file
  fs.readFile("data.json", "utf-8", (err, data) => {
    if (err) {
      res.json({
        status: "failure",
        info: "Failed to get the data from file",
      });
    } else {
      data = JSON.parse(data);
      data.results.forEach((ele, index) => {
        if (ele.id == Number(req.params.id)) {
          ...
          fs.writeFile("data.json", JSON.stringify(data), (err) => {
            if (err) {
              res.json({
                status: "failure",
                info: "Failed to add updated data in the file",
              });
            } else {
              console.log("deleted the record");
              res.json({
                status: "success",
                info: "Deleted Successfully the User",
              });
            }
          });
        }
